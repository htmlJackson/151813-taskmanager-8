...
mainFilter.insertAdjacentHTML(`beforeend`, `
  ${getFilterElement(`all`, `15`, false, true)}
  ${getFilterElement(`overdue`, `0`, true)}
  ${getFilterElement(`today`, `0`, true)}
  ${getFilterElement(`favorites`, `7`)}
  ${getFilterElement(`repeating`, `2`)}
  ${getFilterElement(`tags`, `6`)}
  ${getFilterElement(`archive`, `115`)}
`);

boardTasks.insertAdjacentHTML(`beforeend`, generateRandomCards());
...
Array.from(filtersList).forEach((elem) => {
  elem.addEventListener(`click`, () => {
    clearBoard();
    boardTasks.insertAdjacentHTML(`beforeend`, generateRandomCards(Math.floor((Math.random() * 10) + 1)));
  });
});
